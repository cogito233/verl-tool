...
Then execute:
    python -m verl_tool.servers.tests.test_LLM_int_1 browser \
        --url=http://localhost:5000/get_observation
"""
...
# Browser tests
# ───────────────────────────────────────────────
def test_LLM_int_1(url: str = "http://localhost:5000/get_observation",
                 trajectory_id: str = "test-1"):
    """
    Fire a couple of minimal actions against the text-1 endpoint.
    """
    ...
    action_str = (
        "Locate Cristiano Ronaldo on Wikipedia\n"
    ...
    extra_fields = [
        {
            "user_system_prompt": "You are a user seeking information about actors.",
            "coach_system_prompt": "Provide accurate information about the queried actors.",
            "id": 41214,
            "index": 41214,
            "split": "train",
            "url": (
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = [
            executor.submit(_send_test_request, url, traj_id, action, extra_field, f"Browser-Smoke-Test-{i+1}")
            for i, (traj_id, action, extra_field) in enumerate(zip(traj_ids, actions, extra_fields))
        ]
