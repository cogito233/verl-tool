...
#                 # ──────────────── ② 记录 old→new 映射 ────────────────
#                 # Save the request_id, timestamp, attempt and request_content
#                 # try:
#                 #     log_dir = Path("/minimax-dialogue/users/ruobai/rl_r2e/vllm_logs")
#                 #     log_dir.mkdir(parents=True, exist_ok=True)
#                 #     with (log_dir / f"request_log_{self.start_time}.jsonl").open("a") as f:
#                 #         f.write(json.dumps({
#                 #             "ts": time.time(),
...
#         # Save prompt, response, request_id, timestamp
#         # try:
#         #     log_dir = Path("/minimax-dialogue/users/ruobai/rl_r2e/vllm_logs")
#         #     log_dir.mkdir(parents=True, exist_ok=True)
#         #     with (log_dir / f"prompt_response_pair_{self.start_time}.jsonl").open("a") as f:
#         #         f.write(json.dumps({
#         #             "ts": time.time(),
#         #             "id": request_id,
#         #             "prompt": prompt,
...
class VerlToolChatCompletionScheduler(ChatCompletionScheduler):
    ...
    def __init__(
        ...
        self.max_response_length = self.agent_config.max_response_length
        self.max_concurrent_trajectories = self.agent_config.max_concurrent_trajectories
        print(f"max_concurrent_trajectories: {self.agent_config.max_concurrent_trajectories}")
        # self.max_concurrent_trajectories = 128
    ...
    async def _completions_aiohttp(self, address: str, **complete_request) -> Completion:
        ...
        # 使用 async with 确保 session 总是被正确关闭
    ...
    async def _submit_completions(
        ...
        try:
            ...
            logger.exception(f"chat completion failed with exception: {e}")
            raise e
        finally:
            # 最终确保 request_id 被清理（防御性编程）
            self.request_id_to_address.pop(request_id, None)
    ...
    async def generate_sequences(self, batch: DataProto, **kwargs) -> DataProto:
        ...
        tasks = []
        try:
            if True: # self.agent_config.enable_agent:
                if self.max_concurrent_trajectories is not None and self.max_concurrent_trajectories > 0:
